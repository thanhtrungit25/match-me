...
import { combinedRegisterSchema, profileSchema, ProfileSchema, RegisterSchema } from "@/lib/schemas/RegisterSchema";
...
export async function registerUser(data: RegisterSchema): Promise<ActionResult<User>> {
  try {
    ...
    if (existingUser) return { status: 'error', error: 'User already exits' };
...
export async function resetPassword(token: string | null, password: string): Promise<ActionResult<string>> {
  try {
    if (!token) {
